...
function get_po_dates($limit,$offset) {
    ...
    $qry = "select date_format(email_datetime,'%Y-%m-%d') as po_date, count(*) from email_conf where email_type like 'Purchase Order #BT%' group by po_date order by po_date desc limit $offset,$limit";
    $key=array();
    $res = $obj->query($qry);    
    while ($data = $obj->fetch($res)) {
