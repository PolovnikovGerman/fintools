...
if (!$res) {
    ...
    $qry = "insert into af_attach(att_ref, att_ch, att_path, att_name, att_type,att_datetime, att_edit) values(0," . $_POST['chid'] . ",'" . $newdoc . "','BLUETRACK_PO_BT" . $_POST['oid'] . $_POST['chpo'] . ".pdf','poart',now()," . $_POST['chid'] . ")";
    $res = $obj->query($qry);
    if (!$res) {
        $error['flag'] = true;
        ...
        if (!$res) {
            ...
            if (!$res) {
                ...
                $qry = "update af_child set ch_placed_ck = 'yes', ch_ship_date = '" . format_date($_POST['r2_ship_date']) . "'"
                        . ", ch_vendor = '" . $data['v_abbr'] . "', ch_poTotal = " . floatval(str_replace(',','',$_POST['r2_grand_total'])) . ", "
                        . "ch_active = 'on' , ch_printqty=".intval($_POST['ch_printqty']).", "
                ...
                if (!$res) {
                    ...
                    $last_id = $_POST['chid'];
                    // Update AF Master
                    $qry = "update af_master set  af_itemqty=".intval($_POST['af_itemqty'])." where af_order_id = " . $_POST['af_order_id'];
                    $res = $obj->query($qry);
                    if (!$res) {
                        $error['flag'] = true;
                        $error['msg'][] = "update af_master query failed for PO " . $_POST['oid'] . $_POST['chpo'] . "<br>";
                    } else {
                        $go = 0;
...
//art text message
        ...
        $save_name = "../docs/art_af_fl_uploads/poart/BLUETRACK_PO_BT" . $details['oid'] . $details['chpo'] . ".pdf";
