...
left join af_vendor v on v.v_abbr=chld.ch_vendor
where m.af_date!='0000-00-00' and v.v_type != 'chinese' order by m.af_order_id";
$res = $obj->query($qry);
$results=array();
...
$filename='../uploads/fullfill_export_'.date('Y-m-d').'_'.date('H_i_s').'.csv';
$fh=fopen($filename,'w');
if ($fh) {
    /* Title */
