<?php include('../controller/brandSB_controller.php'); ?>
...
<?php include("../includes/header_navigation.php"); ?>
 ...
 <div class="content">
 	...
 	<div style="float:left; margin-left:0px; z-index:999; margin-top:9px;"><a href="brandSB.php"><img   src="../images/brand_SB_a.png" /></a><a href="brandBU.php"><img style="border-bottom:1px #0D0D0D solid; "  src="../images/brand_BU_ia.png" /></a>
  	</div>
 	<div class="clear"></div>
  
  	<div style="width:960px; margin:auto; text-align:center; margin-top:-1px;  border:1px #232323 solid; height:730px; ">
		<div class="right" style="margin:5px;"><a href="#?w=750&v=add&id=0&org=open" rel="popup_name" class="poplight popUp_addNew" >add New</a></div>
    	<div class="clear"></div>
    	<div><input name="sbCatID" type="hidden" value="0" />
        	
          

			<div class="left categoryPane">
            <table class="categoryTable" style="margin-top:18px;" width="100%" cellpadding="0" cellspacing="0" >
				<?php
					for($i = 0; $i < sizeof($brandSB['sbCat']); $i++)
						echo "<tr><td class=sbCategory id=".$brandSB['sbCat'][$i]['sbCatID'].">".$brandSB['sbCat'][$i]['sbCatName']."</td></tr>";
				?>
            </table>
			</div>

<!------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- -->
<div class="left sbCatSort">
...
<!------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- -->
<div class="right sbItemsList" id="sbItems"   >
        	<table border="0" width="100%" cellpadding="0" cellspacing="0" ><tr><td colspan=5 class="noCat">Highlited Items <b>have</b> IMAGE(s)</td></tr>
        	<?php
				for($i=0 ; $i < sizeof($brandSB['sbID']); $i++)
				{
				if($brandSB['sbCatCount'][$i] <= 0 || $brandSB['sbCatCount'][$i] == NULL)
					$catCount = 'noCat';
					else
					$catCount = '';
					
				if($brandSB['sbNew'][$i] == 'yes')
					echo "<tr class=\"".$brandSB['img'][$brandSB['sbID'][$i]]." hilite\" ".col_bg('#f5f5f5',($i+1))."><td>".($i+1)."</td><td><input type=checkbox class=sbCheck name=sbItem value=".$brandSB['sbID'][$i]."></td><td><b>!</b><td><a href=\"#?w=750&v=edit&id=".$brandSB['sbID'][$i]."&org=open\" rel=\"popup_name\" class=\"poplight\" >".$brandSB['sbItemID'][$i]."</a></td><td>".$brandSB['sbName'][$i]."</td></tr>";
				else
					echo "<tr class=\"".$brandSB['img'][$brandSB['sbID'][$i]]." hilite\" ".col_bg('#f5f5f5',($i+1))."><td>".($i+1)."</td><td><input type=checkbox class=sbCheck name=sbItem value=".$brandSB['sbID'][$i]."></td><td><td><a href=\"#?w=750&v=edit&id=".$brandSB['sbID'][$i]."&org=open\" rel=\"popup_name\" class=\"poplight\" >".$brandSB['sbItemID'][$i]."</a></td><td>".$brandSB['sbName'][$i]."</td></tr>";
				}
			?>
            </table>
			...
			<!-- DATA DIV -->	
                <div style="margin:10px; " class="SBData">
                    ...
                    <div style="float:right; width:240px;  margin-left:5px;  font-size:12px;">
                	    <table>
        			<tr><td>Vendor</td><td><input type="text" size="26" name="sbVendor"/></td></tr>
